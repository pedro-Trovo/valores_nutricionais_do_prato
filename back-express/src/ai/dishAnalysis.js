...
export async function getDishAnalysis(imagePath, tablePath) {
  ...
  const prompts = [
    ...
    'Analise a image e liste os macronutrientes (Gordura, Carboidrato, Proteína, nessa ordem) da refeição, retornando os dados apenas e unicamente em formato JSON (nada de texto além do JSON), sendo que o JSON deverá conter um array, onde cada campo do array conterá o atributo "macronutriente" (armazena o nome do macronutriente) e a "porcentagem" (armazena a porcentagem de cada macronutriente em valor numerico).',

    'Analise os alimentos que compõem a imagem e veja se as calorias deles somadas ultrapassem 800 calorias (kcal). Caso ultrapasse 800 calorias, a refeição não é saudável. Caso não ultrapasse 800 calorias, a refeição é saúdavel. Diga se a refeição é saudável ou não, retornando os dados apenas e unicamente em formato JSON (nada de texto além do JSON), sendo que o JSON deverá conter um array, onde o único campo do array conterá o atributo "ehSaudavel" e o valor dele será um boolean (true para saúdavel, e false para não saúdavel).',
  ];
